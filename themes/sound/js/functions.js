...
async function startUploadAdmin(event) {
	...
	ajax.addEventListener("error", errorHandler, false);
	ajax.addEventListener("abort", abortHandler, false);
	await ajax.open("POST", baseUrl+"/requests/post_track_admin.php");
...
function upViews(){
	var selectUpDown = $("#selectUpDown").val();
	window.location.href = baseUrl+"/index.php?a=explore&type="+selectUpDown+"&order=asc";
}
function downViews(){
	var selectUpDown = $("#selectUpDown").val();
	window.location.href = baseUrl+"/index.php?a=explore&type="+selectUpDown+"&order=desc";
