...
class feed {
	...
	function explore($start, $value, $column = 'views', $orderBy = 'DESC') {
		if($start == 0) {
			$start = '';
		} else {
			// Else, build up the query
		...
		$order = " ORDER BY `tracks`.`views` ".$orderBy.", `tracks`.`likes` ".$orderBy." ";
		if($column == 'views'){
			$order = " ORDER BY `tracks`.`views` ".$orderBy;
		}elseif($column == 'like'){
			$order = " ORDER BY `tracks`.`likes` ".$orderBy;
		}
		...
		if(isset($_GET['popular'])) {
			...
			$query = sprintf("SELECT * FROM `tracks`, `users` WHERE `tracks`.`tag` REGEXP '[[:<:]]%s[[:>:]]' AND `tracks`.`uid` = `users`.`idu` %s AND `tracks`.`public` = '1' AND `users`.`private` = 0 ".$order." LIMIT %s", $this->db->real_escape_string($value), $start, ($this->per_page + 1));
			$value = '\''.$value.'\'';
		} else {
			$query = sprintf("SELECT * FROM `tracks`, `users` WHERE `tracks`.`uid` = `users`.`idu` %s AND `tracks`.`public` = 1 AND `users`.`private` = 0 ".$order." LIMIT %s", $start, ($this->per_page + 1));
			$value = '\'\'';
		}
		// var_dump($query);
		// die;
	...
	function sidebarDescription($id, $type = null, $raw = null) {
		...
		if($type !== 1) {
			...
			$date = !empty($result[2]) ? explode('-', $result[2]) : [];
	...
	function getActions($id, $type = null, $url = null) {
		...
		if($this->l_per_post) {
			...
			$queryListen = sprintf("SELECT distinct username, first_name, last_name, image FROM `views`, `users` WHERE `views`.`track` = '%s' and `views`.`by` = `users`.`idu` LIMIT %s", $this->db->real_escape_string($id), $this->db->real_escape_string($this->l_per_post + 1));
			$arrayListen = [];
			$resultListen = $this->db->query($queryListen);
			while($row = $result->fetch_assoc()) {
				$array[] = $row;
			}
			while($row = $resultListen->fetch_assoc()) {
				$arrayListen[] = $row;
			}
			$i = 0;
		...
		$views_stats = '';
		if($get['views']){
			$views_stats = '<a href="'.permalink($this->url.'/index.php?a=track&id='.$id.'&name='.cleanUrl($get['title'])).'" rel="loadpage" class="music_views_counter"><div class="counter views_counter">'.$get['views'].'</div><span class="list_user">';
			foreach($arrayListen as $row) {
				$views_stats .= '<p><img src="'.permalink($this->url.'/image.php?t=a&w=25&h=25&src='.$row['image']).'" />'. realName($row['username'] ?? '', $row['first_name'] ?? '', $row['last_name'] ?? '').'</p>';
			}
			$views_stats .= '</span></a>';
		}
...
function admin_stats($db, $type, $values = null) {
	if($type == 1) {
		...
		(SELECT count(idu) FROM `users`) as users_today,
		...
		(SELECT COUNT(id) FROM `playlists`) AS playlists_today,
		...
		(SELECT COUNT(id) FROM `tracks`) AS tracks_today,
		...
		(SELECT COUNT(id) FROM `comments`) AS comments_today,
		...
		(SELECT count(id) FROM `likes`) as likes_today,
		...
		(SELECT COUNT(id) FROM `downloads`) AS downloads_today,
		...
		$db->real_escape_string($values['currency']), $db->real_escape_string($values['currency']), $db->real_escape_string($values['currency']), $db->real_escape_string($values['currency']));
	}
